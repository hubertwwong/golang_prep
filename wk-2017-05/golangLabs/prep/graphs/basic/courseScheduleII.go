...
func (g *Graph) hasCycle() bool {
  for i := 0 ; i < len(g.vertexes) ; i++ {
    visited := make([]int, 0)
    //visited[0] = i
    hasCycle := g.hasCycleDFS(i, visited)
    //fmt.Println("> hasCycle ", i, ">", hasCycle) 
    if hasCycle {
      return true
    }
  }
...
func (g *Graph) hasCycleDFS(v int, visited []int) bool {
  //fmt.Println("\n> hasCycleDFS > v", v, "> visited >", visited)
  if intInSlice(v, visited) {
    return true
  } else {
    hasCycle := false
    adj := g.adj(v)
    for i := 0 ; i < len(adj) ; i++ {
      ...
      hasCycle = hasCycle || g.hasCycleDFS(adj[i], visited)
...
func (g *Graph) topSort() []int {
  ...
  // compute if it does not have a cycle.
  if !g.hasCycle() {
    for ; len(unvisited) > 0 ; {
